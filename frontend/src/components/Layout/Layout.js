...
const Layout = () => {
  ...
  // Check if current path is an admin route or finance route
  ...
  // Only show sidebar for admin or finance users, or on admin/finance routes
  const shouldShowSidebar = isAdminRoute || isFinanceRoute || isAdmin || isFinance || isMLEngineer;
  
  return (
    <div className="flex flex-col h-screen bg-gray-50 dark:bg-gray-900 transition-colors duration-200">
      {/* Only show top Navbar for routes that aren't admin or finance */}
      {!isAdminRoute && !isFinanceRoute && <Navbar />}
      
      <div className="flex flex-1 overflow-hidden">
        {/* Sidebar (only visible for admin/finance users or on admin/finance routes) */}
